...
class WeatherLLMParser:
    ...
    def parse(self, text: str) -> Optional[Dict[str, Any]]:
        ...
        system_prompt = (
            ...
            "\n"
            "重要规则：\n"
            "- API返回的天数是从今天开始计算的，days=1表示今天，days=2表示今天+明天\n"
            "- 如果用户说\"今天\"或没提时间，返回days=1（查询今天）\n"
            "- 如果用户说\"明天\"，返回days=2（查询今天+明天，但实际要显示明天的）\n"
            "- 如果用户说\"后天\"，返回days=3（查询今天+明天+后天，但实际要显示后天的）\n"
            "- 如果用户说\"明后天\"或\"明后两天\"，返回days=2（查询今天+明天，但实际要显示明天和后天的）\n"
            "- 如果没说城市，默认城市为\"北京\"\n"
            "- 如果没说天数，默认天数为1（今天）\n"
            "- 城市名示例：上海、北京、广州、深圳、杭州\n"
            "- 天数解析：三天=3、未来一周=7、一周=7、明后天=2、明天=2、后天=3、今天=1\n"
            "\n"
            "示例：\n"
            "输入：上海天气 → {\"city\": \"上海\", \"days\": 1}\n"
            "输入：上海今天天气 → {\"city\": \"上海\", \"days\": 1}\n"
            "输入：北京明天天气 → {\"city\": \"北京\", \"days\": 2}\n"
            "输入：北京后天天气 → {\"city\": \"北京\", \"days\": 3}\n"
