...
from features import ScheduleCommandHandler, NewsCommandHandler, FestivalCommandHandler, MessageBoardCommandHandler, WeatherCommandHandler, ScheduleCategory
...
class VoiceAssistant:
    def __init__(self, mock_mode: bool = False):
        ...
        # 多轮对话状态管理
        self.dialog_state = None  # None 或 "waiting_schedule_continue"
        self.pending_schedule_data = {}  # 存放等待用户确认的数据
        ...
        try:
            ...
            # 初始化留言板处理器（需要speaker_recognizer）
    ...
    def _build_reminder_text(self, item):
        """生成提醒话术：优先使用为该日程预先生成的 LLM 文本。"""
        # 若日程本身带有 LLM 生成的提醒语句，优先使用
    ...
    def _handle_schedule_continue(self, text: str) -> Optional[str]:
        ...
        remaining_items = remaining_items[displayed_count:]  # 跳过已显示的
    ...
    def _check_festival_reminders(self):
        ...
        try:
            ...
            if festival_reminder:
                ...
                # 等待提醒播放完成
                import time
                time.sleep(2)  # 简单等待，实际应该等待TTS播放完成
        except Exception as e:
